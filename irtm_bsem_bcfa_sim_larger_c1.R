...
## Breaking into 6 parts to run independently 
## b.c have 8 cores
library(MASS)
library(mvtnorm)
library(coda)
...
larger_sim <- NKd_vals[which(NKd_vals$Var1 %in% c(50, 250, 1000) & # N items
...
## Transforming the 500 into 20. 
## Otherwise keeping the same to have the parameter scaffold.

larger_sim[which(larger_sim$Var2==250), "Var2"] <- 5
larger_sim[which(larger_sim$Var2==500), "Var2"] <- 20
...
chain = 1
## verify with a 50 iteration run:

range = c(1:50)
  
for(i in range){ ## open for loop
#for(i in 1:nrow(larger_sim)){ ## open for loop
...
## Save
rname = paste0("irtm_bcfa_bsem_results_", ".rds")
tname = paste0("irtm_bcfa_bsem_time_", ".rds")
save(all_results, file=rname)
save(model_times, file=rname)

#fname = paste0("irtm_bcfa_bsem_comp_", Sys.Date(), ".rds")
#save(all_results, file=fname)

## Process the results:
#sim_results <- bsem_results_to_df(all_results, larger_sim)
