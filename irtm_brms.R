...
source('brms_and_cfa_model_syntax.R') ## script to format CFA model input
...
tpartname = "irtm_brms_time_partial_.rds"
ppartname = "irtm_brms_params_partial_.rds"
rpartname = "irtm_bcfa_res_partial_.rds"
...
## Single run:
param <- single_pass_no_dupes[311, ]
## 311 is N = 250, d = 5
sim_data <- gen_sim_dat(param, lambda_zero_pct)
...
brms_irt_model <- brms_irt_format(M, dims= d, vars= feature_names)
brms_latent_var <- brms_latent_format(M, dims= d, vars= feature_names)
brms_irt_model
brms_latent_var
