...
## all_results:
#load("simulations/irtm_only_time_partial_.rds")
## model_times:
#load("simulations/irtm_only_res_partial_.rds")

## full:
load("simulations/irtm_only_results_single_pass.rds")
...
gg_remainder <- ggplot(na.omit(na_count_by_group), 
  ...
  theme(legend.position="bottom", 
        legend.background = element_rect(color='black'), 
        text=element_text(size=20, family="Times")) +
  facet_wrap(~ pass_N + pass_K, labeller = label_both)
